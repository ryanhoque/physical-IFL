...
class CRUAllocation(Allocation):
    """
    An allocation strategy that prioritizes robots by (C)onstraint Violations, (S)afety, and (U)ncertainty
    """
    def allocate(self, allocation_metrics):
        ...
        num_to_free = 0 # number of robots that have very low priority
        free_idx = set()
        priorities = list()
        num_violating = 0
